...
export function normalizeStatus(payload) {
  ...
  return {
    ...
    overallProgress: Number(payload?.overall_progress || 0),
    ...
    activityFeed: Array.isArray(payload?.activity_feed) ? payload.activity_feed : [],
    stages: Array.isArray(payload?.stages) ? payload.stages : [],
    renderStatus: payload?.render_status || { active: false, progress: 0, message: "" },
    uploadStatus: payload?.upload_status || { active: false, progress: 0, message: "" },
    qualityStatus: payload?.quality_status || { active: false, progress: 0, message: "" },
